...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^velvet_") ; then
       CUSTOM_BUILD=$(echo -n $1 | sed -e 's/^velvet_//g')
    else
       CUSTOM_BUILD=
    fi
    export CUSTOM_BUILD
...
function breakfast()
{
    ...
    unset LUNCH_MENU_CHOICES
    for f in `/bin/ls vendor/velvet/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the target model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch velvet_$target-$variant
        fi
    fi
    return $?
}
...
function mk_timer()
{
    ...
    if [ $ret -eq 0 ] ; then
		...
		paplay $(gettop)/vendor/velvet/utils/sounds/failed.wav &> /dev/null #hide error messages
    fi
    if [ $hours -gt 0 ] ; then
